...
static inline long sbi_probe_extension(int extid) {
  ...
  ret = sbi_ecall(SBI_EXT_ID_BASE, SBI_BASE_PROBE_EXTENSION, extid, 0, 0, 0, 0,
                  0);
  if (ret.error) {
    return ret.error;
  }
...
static inline long sbi_hart_start(uint64 hartid, uint64 saddr, uint64 priv) {
  ...
  ret = sbi_ecall(SBI_EXT_ID_HSM, SBI_HSM_HART_START, hartid, saddr, priv, 0, 0,
                  0);
  if (ret.error) {
    return ret.error;
  }
...
void sbiinit(void) {
  ...
  printf("\n");
  // Enable interrupts after SBI initialization.
